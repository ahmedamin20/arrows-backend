const Staff = require('../../models/ourTeam');
...
const create = (req, res, next) => {
    ...
    Staff.create({ ...data, ...(req.file ? { imageUrl: req.file.path } : {}) })
...
const readOne = (req, res, next) => Staff.findById(req.params.id)
...
const update = (req, res, next) => {
    const data = JSON.parse(req.body.data)
    Staff.updateOne({ _id: req.params.id }, { ...data, ...(req.file ? { imageUrl: req.file.path } : {}) })
