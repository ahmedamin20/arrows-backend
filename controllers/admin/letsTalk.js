...
const create = (req, res, next) => {
    LetsTalk.create(req.body).then(result => res.status(202).end())
...
const readAll = async (req, res, next) => {
    ...
    LetsTalk.paginate(filter ? ({ $text: { $search: `"${filter}"` } }) : {}, options)
        .then(result => { res.json({ items: result.docs, count: result.total }) })
        .catch((err) => next(fireError(err, 'Failed to read.', 500)));
}

const readOne = (req, res, next) => LetsTalk.findById(req.params.id)
    .then(item => item ? res.json(item) : res.status(404).end())
    .catch((err) => next(fireError(err, 'Failed to read.', 500)));

const update = (req, res, next) =>
    LetsTalk.updateOne({ _id: req.params.id }, req.body).then(result => res.status(204).end())
        .catch((err) => next(fireError(err, 'Failed to update.', 500)));


const remove = async (req, res, next) =>
    LetsTalk.deleteOne({ _id: req.params.id }).then(result => res.status(204).end())
