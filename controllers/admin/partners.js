...
const create = (req, res, next) => {
    ...
    Partners.create({ ...data, ...(req.file ? { imageUrl: req.file.path } : {}) })
...
const readAll = async (req, res, next) => {
    try {
        ...
        const result = await Partners.paginate(query, options);
...
const readOne = (req, res, next) => Partners.findById(req.params.id)
...
const update = (req, res, next) => {
    const data = JSON.parse(req.body.data)
    Partners.updateOne({ _id: req.params.id }, { ...data, ...(req.file ? { imageUrl: req.file.path } : {}) })
