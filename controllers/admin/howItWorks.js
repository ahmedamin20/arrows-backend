...
const create = (req, res, next) => {
    ...
    HowItWorks.create({ ...data, ...(req.file ? { imageUrl: req.file.path } : {}) })
        .then(result => res.status(202).end()).catch(err => (err.code === 11000) ?
            next(fireError(err, 'Failed, Parnter already exists.', 409)) :
...
const readAll = async (req, res, next) => {
    try {
        ...
        const result = await HowItWorks.paginate(query, options);
...
const readOne = (req, res, next) => HowItWorks.findById(req.params.id)
...
const update = (req, res, next) => {
    const data = JSON.parse(req.body.data)
    HowItWorks.updateOne({ _id: req.params.id }, { ...data, ...(req.file ? { imageUrl: req.file.path } : {}) })
        .then(result => res.status(204).end()).catch((err) => next(fireError(err, 'Failed to update.', 500)));
}

const remove = async (req, res, next) => {
    try {
        const doc = await HowItWorks.findById(req.params.id);
        await doc.deleteOne();
        res.status(204).end();
    } catch (err) {
